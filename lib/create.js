...
const create = (name, { O0 }) => {
    ...
    .then(swiftenv => {
        const next = (optimize) => exec(`${swiftenv} versions`, (err, out, stdErr) => {
            if (err != null) {
                console.error(err);
                process.exit(1);
            } else {
                if (out.indexOf(release.trimmedName) != -1) init(name, path, release.trimmedName, optimize);
                else install(name, path, release).then(_ => init(name, path, release.trimmedName, optimize));
            }
        });
        if (!O0) {
            commandExists('wasm-strip')
            .then(_ => next(true))
            .catch(err => {
                console.log("You do not have wabt installed.\n".red);
                console.log("It is required to optimize your project. Please install it from https://github.com/WebAssembly/wabt");
                console.log(`On macOS, simply run ${"brew install wabt".cyan.bold}\n`);
                console.log("                 OR\n".bold);
                console.log(`Rerun this command with the ` + '-O0'.bold + ' flag');
            });
        } else {
            next(false);
        }
...
const init = async (name, path, release, optimize) => {
    ...
    await fs.writeFile(`${path}/package.json`, JSON.stringify(makePackageJSON(name, optimize), null, 4));
    ...
    if (optimize) {
        ...
        await fs.copyFile(`${__dirname}/resources/index_unoptimized.js`, `${path}/src/index.js`);
    }
    ...
    await fs.writeFile(`${path}/Makefile`, makeMakefile(name, optimize));
    ...
    exec(`swift package init --type executable`, { cwd: `${path}/${name}` }, async (_, stdout, stderr) => {
        ...
        console.log(`cd ${paramCase(name)} && npm install`.green.bold, 'to finish setting up the project.');
...
const makePackageJSON = (name, optimize) => ({
    "name": paramCase(name),
    "version": "1.0.0",
    "dependencies": {
        "@wasmer/wasi": "^0.9.1",
        ...
        "pako": optimize ? "^1.0.11" : undefined
    },
    "devDependencies": {
        "webpack": "^4.42.0",
        "webpack-cli": "^3.3.11",
...
const makeMakefile = (name, optimize) => `build:
\tcd ${name} && \\
\tswift build --triple wasm32-unknown-wasi && \\
\tcp .build/debug/${name} ../dist/SwiftWASM.wasm${optimize ? " && \\\n\twasm-strip ../dist/SwiftWASM.wasm && \\\n\tgzip ../dist/SwiftWASM.wasm --best" : ""}`;
